...
export const GoogleMapsNavigationMode: React.FC<GoogleMapsNavigationModeProps> = ({
  ...
  if (!userPos || !renderedPos) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center bg-surface text-on-surface p-6 text-center select-none">
        <Loader2 className="w-9 h-9 text-primary animate-spin mb-3" />
        <h3 className="text-sm font-black text-on-surface">{t('nav.acquiringResponderLocation')}</h3>
  ...
  return (
    <div className="relative w-full h-full overflow-hidden flex flex-col select-none touch-none bg-slate-100">
      ...
      <div className="absolute bottom-3 left-3 right-3 z-[500] bg-white/95 backdrop-blur-xl text-slate-900 rounded-3xl p-3 shadow-2xl border border-slate-200/80 animate-in fade-in slide-in-from-bottom-4 duration-300">
        {destinationType === 'hospital' ? (
