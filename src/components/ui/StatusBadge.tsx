...
export const StatusBadge: React.FC<StatusBadgeProps> = ({ status }) => {
  const getBadgeStyle = () => {
    switch (status) {
      case 'EN_ROUTE':
        return 'bg-secondary-fixed text-secondary-fixed-variant border-secondary/30';
      case 'ON_SCENE':
  ...
  return (
    <span className={`px-2.5 py-1 text-xs font-semibold rounded-full border shadow-xs inline-flex items-center gap-1.5 ${getBadgeStyle()}`}>
